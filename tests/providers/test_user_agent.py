...
from randum.providers.user_agent import Provider as UaProvider
...
class TestUserAgentProvider:
    ...
    def test_android_platform_token(self, randum, num_samples):
        for _ in range(num_samples):
            match = self.android_token_pattern.fullmatch(randum.android_platform_token())
            assert match.group('android_version') in UaProvider.android_versions

    def test_ios_platform_token(self, randum, num_samples):
        for _ in range(num_samples):
            match = self.ios_token_pattern.fullmatch(randum.ios_platform_token())
    ...
    def test_mac_platform_token(self, randum, num_samples):
        for _ in range(num_samples):
            match = self.mac_token_pattern.fullmatch(randum.mac_platform_token())
