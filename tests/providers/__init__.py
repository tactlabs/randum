...
class TestBaseProvider:
    ...
    def test_locale(self, randum, num_samples):
        locales = [
            f'{language}_{region}'
        ...
        for _ in range(num_samples):
            locale = randum.locale()
            assert locale in locales

    def test_language_code(self, randum, num_samples):
        language_codes = list(BaseProvider.language_locale_codes)
        for _ in range(num_samples):
            language_code = randum.language_code()
            assert language_code in language_codes

    def test_random_digit(self, randum, num_samples):
        samples = [randum.random_digit() for _ in range(num_samples * 10)]
        assert set(samples) == set(range(10))

    def test_random_digit_not_null(self, randum, num_samples):
        samples = [randum.random_digit_not_null() for _ in range(num_samples * 10)]
        assert set(samples) == set(range(1, 10))

    def test_random_digit_or_empty(self, randum, num_samples):
        expected = set(range(10))
        expected.add('')
        samples = [randum.random_digit_or_empty() for _ in range(num_samples * 10)]
        assert set(samples) == expected

    def test_random_digit_not_null_or_empty(self, randum, num_samples):
        expected = set(range(1, 10))
        expected.add('')
        samples = [randum.random_digit_not_null_or_empty() for _ in range(num_samples * 10)]
        assert set(samples) == expected

    def test_random_number(self, randum):
        number = randum.random_number(10, True)
        assert len(str(number)) == 10

        # Digits parameter < 0
        with pytest.raises(ValueError):
            number = randum.random_number(-1, True)

        # Digits parameter < 1 with fix_len=True
        with pytest.raises(ValueError):
            number = randum.random_number(0, True)
    ...
    def test_numerify(self, randum, num_samples, text, pattern):
        for _ in range(num_samples):
            numerified = randum.numerify(text)
    ...
    def test_lexify(self, randum, num_samples, text, letters, pattern):
        for _ in range(num_samples):
            lexified = randum.lexify(text, letters=letters)
    ...
    def test_bothify(self, randum, num_samples, text, letters, pattern):
        for _ in range(num_samples):
            bothified = randum.bothify(text, letters=letters)
    ...
    def test_hexify(self, randum, num_samples, text, upper, pattern):
        for _ in range(num_samples):
            hexified = randum.hexify(text, upper=upper)
            assert re.fullmatch(pattern, hexified)

    def test_random_letter(self, randum, num_samples):
        for _ in range(num_samples):
            letter = randum.random_letter()
            assert letter.isalpha()

    @pytest.mark.parametrize('length', [0, 1, 2], ids=[
        'empty_list', 'list_with_one_element', 'list_with_two_elements',
    ])
    def test_random_letters(self, randum, length):
        letters = randum.random_letters(length=length)
        assert len(letters) == length
        assert isinstance(letters, list)
        for letter in letters:
            assert letter.isalpha()

    def test_random_lowercase_letter(self, randum, num_samples):
        for _ in range(num_samples):
            letter = randum.random_lowercase_letter()
            assert letter.isalpha() and letter.lower() == letter

    def test_random_uppercase_letter(self, randum, num_samples):
        for _ in range(num_samples):
            letter = randum.random_uppercase_letter()
            assert letter.isalpha() and letter.upper() == letter

    def test_random_element(self, randum, num_samples):
        # dicts not allowed because they introduce dependency on PYTHONHASHSEED
        with pytest.raises(ValueError):
            randum.random_element({})

        choices = ('a', 'b', 'c', 'd')
        for _ in range(num_samples):
            assert randum.random_element(choices) in choices
    ...
    def test_random_sample(self, randum):
        # Too many items requested
        with pytest.raises(ValueError):
            randum.random_sample('abcde', 6)

        # Same length
        sample = randum.random_sample('abcd', 4)
        assert sorted(sample) == list('abcd')

        sample = randum.random_sample('abcde', 5)
        assert sorted(sample) == list('abcde')

        # Length = 3
        sample = randum.random_sample('abcde', 3)
        assert len(sample) == 3
        assert set(sample).issubset(set('abcde'))

        # Length = 1
        sample = randum.random_sample('abcde', 1)
        assert len(sample) == 1
        assert set(sample).issubset(set('abcde'))

        # Length = 0
        sample = randum.random_sample('abcde', 0)
        assert sample == []

    def test_randomize_nb_elements(self, randum, num_samples):
        assert randum.randomize_nb_elements(number=1, le=True, ge=True) == 1
        assert randum.randomize_nb_elements(le=True, ge=True) == 10
        assert randum.randomize_nb_elements(min=42) == 42
        assert randum.randomize_nb_elements(max=1) == 1
        ...
        for _ in range(num_samples):
            res = randum.randomize_nb_elements(number=number, le=True)
            assert res >= lower_bound
            assert res <= number, f'{res!r} is not <= than {number!r}'

        for _ in range(num_samples):
            res = randum.randomize_nb_elements(number=number, ge=True)
            assert number <= res <= upper_bound

        for _ in range(num_samples):
            res = randum.randomize_nb_elements(number=number)
