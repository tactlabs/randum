...
from randum.providers.company.el_GR import Provider as ElGrCompanyProvider
from randum.providers.company.en_PH import Provider as EnPhCompanyProvider
from randum.providers.company.fil_PH import Provider as FilPhCompanyProvider
from randum.providers.company.hu_HU import Provider as HuHuCompanyProvider
from randum.providers.company.hy_AM import Provider as HyAmCompanyProvider
from randum.providers.company.it_IT import Provider as ItItCompanyProvider
from randum.providers.company.ja_JP import Provider as JaJpCompanyProvider
from randum.providers.company.nl_NL import Provider as NlNlCompanyProvider
from randum.providers.company.pl_PL import Provider as PlPlCompanyProvider
from randum.providers.company.pl_PL import company_vat_checksum, local_regon_checksum, regon_checksum
from randum.providers.company.pt_BR import company_id_checksum
from randum.providers.company.ro_RO import Provider as RoRoCompanyProvider
from randum.providers.company.ru_RU import Provider as RuRuCompanyProvider
from randum.providers.company.ru_RU import calculate_checksum
from randum.providers.company.th_TH import Provider as ThThCompanyProvider
from randum.providers.company.tr_TR import Provider as TrTrCompanyProvider
...
class TestFiFi:
    ...
    def test_company_business_id(self, randum, num_samples):
        for _ in range(num_samples):
            company_id = randum.company_business_id()
...
class TestHyAm:
    ...
    def test_bs(self, randum, num_samples):
        for _ in range(num_samples):
            bs = randum.bs()
            assert isinstance(bs, str)

    def test_catch_phrase(self, randum, num_samples):
        for _ in range(num_samples):
            catch_phrase = randum.catch_phrase()
            assert isinstance(catch_phrase, str)

    def test_company(self, randum, num_samples):
        for _ in range(num_samples):
            company = randum.company()
            assert isinstance(company, str)

    def test_company_suffix(self, randum, num_samples):
        for _ in range(num_samples):
            suffix = randum.company_suffix()
...
class TestJaJp:
    ...
    def test_company_prefix(self, randum, num_samples):
        for _ in range(num_samples):
            prefix = randum.company_prefix()
            assert isinstance(prefix, str)
            assert prefix in JaJpCompanyProvider.company_prefixes

    def test_company_category(self, randum, num_samples):
        for _ in range(num_samples):
            category = randum.company_category()
...
class TestPtBr:
    ...
    def test_company_id(self, randum, num_samples):
        for _ in range(num_samples):
            company_id = randum.company_id()
            assert re.fullmatch(r'\d{14}', company_id)

    def test_cnpj(self, randum, num_samples):
        for _ in range(num_samples):
            cnpj = randum.cnpj()
...
class TestPlPl:
    ...
    def test_regon(self, randum, num_samples):
        for _ in range(num_samples):
            assert re.fullmatch(r'\d{9}', randum.regon())
    ...
    def test_local_regon(self, randum, num_samples):
        for _ in range(num_samples):
            assert re.fullmatch(r'\d{14}', randum.local_regon())
    ...
    def test_company_vat(self, randum, num_samples):
        for _ in range(num_samples):
            assert re.fullmatch(r'\d{10}', randum.company_vat())
...
class TestNlNl:
    ...
    def test_large_companies(self, randum, num_samples):
        for _ in range(num_samples):
            company = randum.large_company()
            assert isinstance(company, str)
...
class TestEnPh:
    ...
    def test_random_company_noun_chain(self, randum, num_samples):
        for _ in range(num_samples):
            noun_list = randum.random_company_noun_chain().split()
            assert 1 <= len(noun_list) <= 2

    def test_random_company_acronym(self, randum, num_samples):
        for _ in range(num_samples):
            acronym = randum.random_company_acronym()
...
class TestFilPh(TestEnPh):
    ...
    def test_PH_random_good_service_adjective_chain(self, randum, num_samples):
        for _ in range(num_samples):
            adjectives = randum.random_good_service_adjective_chain().split(' at ')
            assert all(
...
class TestRuRu:
    ...
    def test_businesses_inn(self, randum, num_samples):
        for _ in range(num_samples):
            inn = randum.businesses_inn()
            assert len(inn) == 10
            assert calculate_checksum(inn[:9]) == inn[9]

    def test_individuals_inn(self, randum, num_samples):
        for _ in range(num_samples):
            inn = randum.individuals_inn()
            assert len(inn) == 12
            assert calculate_checksum(inn[:10]) == inn[10]
            assert calculate_checksum(inn[:11]) == inn[11]

    def test_businesses_ogrn(self, randum, num_samples):
        max_year = datetime.now().year - 2000
        for _ in range(num_samples):
            ogrn = randum.businesses_ogrn()
            assert len(ogrn) == 13
    ...
    def test_individuals_ogrn(self, randum, num_samples):
        max_year = datetime.now().year - 2000
        for _ in range(num_samples):
            ogrn = randum.individuals_ogrn()
            assert len(ogrn) == 15
            assert ogrn[0] == '3'
            assert 1 <= int(ogrn[1:3]) <= max_year
            assert 1 <= int(ogrn[3:5]) <= 92
    ...
    def test_kpp(self, randum, num_samples):
        for _ in range(num_samples):
            kpp = randum.kpp()
            assert len(kpp) == 9
    ...
    def test_catchphrase(self, randum, num_samples):
        for _ in range(num_samples):
            catchphrase = randum.catch_phrase()
...
class TestItIt:
    ...
    def test_company_vat(self, randum, num_samples):
        for _ in range(num_samples):
            company_vat = randum.company_vat()
...
class TestThTh:
    ...
    def test_company_limited_prefix(self, randum, num_samples):
        for _ in range(num_samples):
            prefix = randum.company_limited_prefix()
            assert isinstance(prefix, str)
            assert prefix in ThThCompanyProvider.company_limited_prefixes

    def test_company_limited_suffix(self, randum, num_samples):
        for _ in range(num_samples):
            suffix = randum.company_limited_suffix()
            assert isinstance(suffix, str)
            assert suffix in ThThCompanyProvider.company_limited_suffixes

    def test_nonprofit_prefix(self, randum, num_samples):
        for _ in range(num_samples):
            prefix = randum.nonprofit_prefix()
            assert isinstance(prefix, str)
