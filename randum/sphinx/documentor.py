...
from randum.config import AVAILABLE_LOCALES
from randum.config import PROVIDERS as STANDARD_PROVIDER_NAMES
from randum.providers import BaseProvider

if os.environ.get("READTHEDOCS", False):
    version = os.environ["READTHEDOCS_VERSION"]
    HOME = Path("/home/docs/checkouts/readthedocs.org/user_builds/randum/checkouts") / version
    DOCS_ROOT = HOME / "docs"
else:
...
def _write_base_provider_docs():
    ...
    with (DOCS_ROOT / 'providers' / 'baseprovider.rst').open('wb') as fh:
        _hide_edit_on_github(fh)
        _write_title(fh, '``randum.providers``')
        _write_includes(fh)
        _write(fh, PROVIDER_AUTODOC_TEMPLATE.format(
            provider_class='randum.providers.BaseProvider',
