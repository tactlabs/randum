...
from randum.config import AVAILABLE_LOCALES, DEFAULT_LOCALE
from randum.sphinx.validator import SampleCodeValidator

logger = logging.getLogger(__name__)
_fake = Randum(AVAILABLE_LOCALES)
_base_provider_method_pattern = re.compile(r'^randum\.providers\.BaseProvider\.(?P<method>\w+)$')
_standard_provider_method_pattern = re.compile(r'^randum\.providers\.\w+\.Provider\.(?P<method>\w+)$')
_locale_provider_method_pattern = re.compile(
    r'^randum\.providers\.\w+'
...
class ProviderMethodDocstring:
    ...
    - If the docstring belongs to a standard provider method, sample usage and output will be
      generated using a `Randum` object in the `DEFAULT_LOCALE`.
    ...
    - Each ``Sample`` will have info on the keyword arguments to pass to the provider method, how many
      times the provider method will be called, and the initial seed value to ``Randum.seed()``.
    """
