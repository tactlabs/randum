...
from randum import VERSION, Randum, documentor, exceptions
from randum.config import AVAILABLE_LOCALES, DEFAULT_LOCALE, META_PROVIDERS_MODULES
...
def print_doc(provider_or_field=None,
    ...
    args = args or []
    output = output or sys.stdout
    fake = Randum(locale=lang, includes=includes)
    ...
    if provider_or_field:
        if '.' in provider_or_field:
            ...
            try:
                ...
                raise ValueError(f'No randum found for "{provider_or_field}({args})"')
        ...
        for provider, randums in formatters:

            print_provider(doc, provider, randums, output=output)
...
class Command:
  ...
  Randum can take a locale as an optional argument, to return localized data. If
  ...
  for the specified provider, the factory falls back to randum's default locale,
...
examples:
  ...
  $ randum -l de_DE address
  Samira-Niemeier-Allee 56
  94812 Biedenkopf

  $ randum profile ssn,birthdate
