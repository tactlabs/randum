...
from randum.exceptions import UnsupportedFeature

from .. import BaseProvider

localized = True

csv.register_dialect('randum-csv', csv.excel, quoting=csv.QUOTE_ALL)
...
class Provider(BaseProvider):
    ...
    def dsv(self, dialect='randum-csv', header=None,
        ...
        The ``dialect`` argument defaults to ``'randum-csv'`` which is the name of a ``csv.excel``
        ...
        string tokens will be passed to  :meth:`pystr_format() <randum.providers.python.Provider.pystr_format>`
    ...
    def csv(self, header=None, data_columns=('{{name}}', '{{address}}'), num_rows=10, include_row_ids=False):
        ...
        :meth:`dsv() <randum.providers.misc.Provider.dsv>` which is used under the hood.
